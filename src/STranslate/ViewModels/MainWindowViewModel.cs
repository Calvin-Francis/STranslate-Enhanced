...
public partial class MainWindowViewModel : ObservableObject, IDisposable
{
    ...
    private async Task ScreenshotTranslateAsync(CancellationToken cancellationToken)
    {
        ...
        // 如果是屏幕覆盖模式，使用带坐标的截图方法
        if (Settings.ScreenshotTranslateInImage && Settings.UseScreenOverlayMode)
        {
            var result = await _screenshot.GetScreenshotWithBoundsAsync();
            if (result.Bitmap == null) return;

            await ScreenshotTranslateWithBoundsAsync(result, ocrPlugin, cancellationToken);
            return;
        }
    ...
    public async Task ScreenshotTranslateHandlerAsync(System.Drawing.Bitmap? bitmap, IOcrPlugin? ocrPlugin = default, CancellationToken cancellationToken = default)
    {
        ...
        {
            // 使用传统弹窗模式（覆盖模式在 ScreenshotTranslateAsync 中已单独处理）
    ...
    /// 屏幕覆盖模式的截图翻译处理（带坐标）
    /// </summary>
    private async Task ScreenshotTranslateWithBoundsAsync(ScreenshotResult screenshotResult, IOcrPlugin? ocrPlugin, CancellationToken cancellationToken)
    {
        if (screenshotResult.Bitmap == null) return;
        ...
        // 打开覆盖窗口
        var window = await SingletonWindowOpener.OpenAsync<ScreenOverlayWindow>();
        var viewModel = (ScreenOverlayWindowViewModel)window.DataContext;

        // 如果有屏幕坐标，设置窗口位置
        if (screenshotResult.HasScreenBounds)
        {
            window.SetScreenBounds(screenshotResult.ScreenBounds);
        }
        else
        {
            // 没有坐标时，居中显示并设置合适的大小
            window.WindowStartupLocation = WindowStartupLocation.CenterScreen;
            window.Width = Math.Max(200, screenshotResult.Bitmap.Width);
            window.Height = Math.Max(100, screenshotResult.Bitmap.Height);
        }

        // 执行翻译
        await viewModel.ExecuteCommand.ExecuteAsync(screenshotResult.Bitmap);
    }
